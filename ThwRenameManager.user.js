        ...
        $('#btnRename').on('click', function() {
            ...
            $('#vehicle_table > tbody').children().each(async function() {
                try {
                    ...
                    var vehicleID = $vehicleNameColumn.find("a")[0].href.split("/")[4];
                    var apiVehicleType = cVehicles.filter(v => v.id == vehicleID)[0];
                    var vehicleType = apiVehicleType['vehicle_type'];
                    var org = "";
                    var type = "";
                    switch (vehicleType){
                        case 39:
